...
{{< alert >}}
 *In my day job I am actually working on the .NET SDK of Realm, so my opinion of it may be somehow skewed :grin:
 {{< /alert >}}
...
For instance, `CopyToRealm` is a method used to add an object to a realm. When this happens, all its properties need to be written too, and this is what the method does. If we look at a simplified implementation for the `Person` class before:
...
In a similar way to what is done in ModuleInit.Fody, Realm is also weaving its own module initializer. This builds the default schema that is used to open a realm when no schema is passed in the configuration. The use of the module initializer allows this process to happen automatically, without the need for Realm users to call any initialization code themselves. 
...
Nevertheless, weaving has some major drawbacks:
-  As I've shown [before](#il-weaving-and-fody), IL code is quite difficult to read and comprehend for developers used to a much higher level of abstraction when programming.
- It requires a very specific knowledge to work with, making it difficult to maintain, especially in the context of a library whose primary target is not weaving-related.
- The generated code cannot be verified by the compiler, and it's not even possible to use the debugger with it. 
- Different weavers can interact involuntarily with each other, leading to unpredictable results.
...
- The generated code is part of the compilation, thus it can be debugged and unit tested.
- Source generators work with strings, that are much easier to deal with than IL instructions and codes. 
- It's much easier to generate code that is using the latest available language features.

In my opinion, these characteristics make source generators much more maintainable than IL weaving, and lead to a greatly improved developer experience. 
